"""A program to see which machines a drawing is on, if any. Let the user know,
and give them the option to open the .mpr file if they choose.

Inputs:
    drawingNumber -- This is the drawing number excluding any revision
    information.
    import from windows clipboard -- This option is automatic. It the
    clipboard is between 6 - 8 numbers long it will prompt with the option
    to press 1 and look up that drawing.

Outputs:
    A printed list of which machines the drawing is on (along with date/time
    of last file modification), if none then it will offer to create file
    using Gen_rec.py. If Gen_rec.py is not available, the user will be
    notified and the program will move on. The option to open the .mpr file
    on one of those machines from the terminal will be there, if it exists.
"""
import os
import time
...
# list of machine pathways, setting 'if it exists' to false by default,
# and machine names
vantechPath, vantech, van = machineFilePaths.vantech_parts(), False, 'Vantech'
jasperWeekePath, jasperWeeke, jas = machineFilePaths.jasper_parts(), False,\
                                    'JasperWeeke'
...
try:
    if int(pyperclip.paste()) >= 6 & int(pyperclip.paste()) <= 8:
        ...
        drawingNumber = input(str('Please enter the drawing number you want '
                                  'to find.\n'))
except ValueError:
    drawingNumber = input(str('Please enter the drawing number you want '
                              'to find.\n'))
...
while drawingNumber:
    ...
    for path in allPaths:
        for rootDir, subDir, filenames in os.walk(path):
            for filename in filenames:
                if (filename.endswith('.mpr') or filename.endswith('.ard')) \
                        and filename.startswith(drawingNumber):
                    for i in range(len(machineNames)):
                        if machineNames[i] in path:
                            fullpath = os.path.join(rootDir, filename)
                            foundOnMachine[i] = True
                            print(str(fileListKey) + ': ' + filename
                                  + ' found on the ' + machineNames[i]
                                  + ' : '
    ...
    # and prompt for next file to look up
    isFileOpen = False
    while any(foundOnMachine):
        fileToOpen = input(str('Type the line number to open the file or just '
                               'hit enter to look up a different file.'))
        if not fileToOpen:
            break
    ...
    drawingNumber = input(str('Please enter the drawing number you want to '
